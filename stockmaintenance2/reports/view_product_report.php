            ...
            COALESCE(
                (
                    SELECT 
                        SUM(s.quantity)
                    FROM sales s
                    WHERE s.product_id = p.product_id AND s.mrp = pm.mrp
                ), 0) AS `Sales`,
            COALESCE(
                (
                    SELECT 
                        SUM(pur.quantity)
                    FROM purchase pur
                    WHERE pur.product_id = p.product_id AND pur.mrp = pm.mrp
                ), 0) AS `Purchase`,
            COALESCE(
                (
                    SELECT 
                        CASE 
                            WHEN SUM(s.quantity) = 0 THEN 'Hold'
                            WHEN SUM(s.quantity) BETWEEN 1 AND 10 THEN 'Light Move'
                            ELSE NULL
                        END
                    FROM sales s
                    WHERE s.product_id = p.product_id AND s.mrp = pm.mrp

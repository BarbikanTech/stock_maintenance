...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT 
            p.unique_id, p.date, p.order_id, p.vendor_id, p.vendor_name, p.mobile_number, p.business_name, p.gst_number, 
            p.address, p.invoice_number, p.created_date, 
            vd.product_id, vd.product_name, vd.sku, vd.quantity, vd.mrp
        FROM purchase p
